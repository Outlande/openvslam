...
void mono_localization(const std::shared_ptr<openvslam::config>& cfg,
                       ...
                       const unsigned int frame_skip, const bool no_sleep,
                       const bool auto_term) {
    // load the mask image
    tum_rgbd_sequence sequence(dataset_path);
...
// and pass the frame_publisher and the map_publisher
    ...
    socket_publisher::publisher publisher(
        openvslam::util::yaml_optional_ref(cfg->yaml_node_, "SocketPublisher"),
        &SLAM, SLAM.get_frame_publisher(), SLAM.get_map_publisher());
#endif
    ...
    std::thread thread([&]() {
        for (unsigned int i = 0; i < frames.size(); ++i) {
            ...
            const auto img = cv::imread(frame.rgb_img_path_, cv::IMREAD_UNCHANGED);

            const auto tp_1 = std::chrono::steady_clock::now();

            if (!img.empty() && (i % frame_skip == 0)) {
                // input the current frame and estimate the camera pose
                SLAM.feed_monocular_frame_localize(img, frame.timestamp_);
            }

            const auto tp_2 = std::chrono::steady_clock::now();

            const auto track_time = std::chrono::duration_cast<std::chrono::duration<double>>(tp_2 - tp_1)
                                        .count();
            if (i % frame_skip == 0) {
            ...
            if (!no_sleep && i < frames.size() - 1) {
                ...
                if (0.0 < wait_time) {
                    std::this_thread::sleep_for(std::chrono::microseconds(
                        static_cast<unsigned int>(wait_time * 1e6)));
                }
            }
...
// run the viewer in the current thread
    ...
    std::cout << "mean tracking time: " << total_track_time / track_times.size()
              << "[s]" << std::endl;
}
    ...
    std::thread thread([&]() {
        for (unsigned int i = 0; i < frames.size(); ++i) {
            ...
            if (!rgb_img.empty() && !depth_img.empty() && (i % frame_skip == 0)) {
                // input the current frame and estimate the camera pose
                SLAM.feed_RGBD_frame_localize(rgb_img, depth_img, frame.timestamp_);
            }
...
int main(int argc, char* argv[]) {
    ...
    auto data_dir_path = op.add<popl::Value<std::string>>(
        "d", "data-dir", "directory path which contains dataset");
    auto config_file_path = op.add<popl::Value<std::string>>("c", "config", "config file path");
    auto map_db_path = op.add<popl::Value<std::string>>(
        "p", "map-db", "path to a prebuilt map database");
    auto mapping = op.add<popl::Switch>(
        "", "mapping", "perform mapping as well as localization");
    auto mask_img_path = op.add<popl::Value<std::string>>("", "mask", "mask image path", "");
    auto frame_skip = op.add<popl::Value<unsigned int>>(
        "", "frame-skip", "interval of frame skip", 1);
    ...
    if (!vocab_file_path->is_set() || !data_dir_path->is_set() || !config_file_path->is_set() || !map_db_path->is_set()) {
    ...
    if (cfg->camera_->setup_type_ == openvslam::camera::setup_type_t::Monocular) {
        mono_localization(cfg, vocab_file_path->value(), data_dir_path->value(),
                          ...
                          frame_skip->value(), no_sleep->is_set(),
                          auto_term->is_set());
    }
